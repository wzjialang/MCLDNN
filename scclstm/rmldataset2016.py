...
def load_data(filename1 = 'F:/code/scclstm/data/RML2016.10b.dat',filename2='F:/code/scclstm/data/RML2016.10a_dict.pkl',train_rate = 0.9):

    Xd1 = pickle.load(open(filename1,'rb'),encoding='iso-8859-1')#Xd1.keys() mod中没有AM-SSB Xd1(120W,2,128)
    Xd2 =pickle.load(open(filename2,'rb'),encoding='iso-8859-1')#Xd2(22W,2,128)
    ...
    mods,snrs = [sorted(list(set([k[j] for k in Xd2.keys()]))) for j in [0,1] ] 
    X = []
    # X2=[]
    lbl = []
    # lbl2=[]
    ...
    for mod in mods:
        for snr in snrs:
            # X2.append(Xd3[(mod,snr)])
            # for i in range (Xd3[(mod,snr)].shape[0]):
            #     lbl2.append((mod,snr))
            if mod=='AM-SSB':
                for i in range(7):
                    X.append(Xd2[(mod,snr)]) 
                for i in range(7*(Xd2[(mod,snr)].shape[0])):
                    lbl.append((mod,snr))
            else:
                X.append(Xd1[(mod,snr)])     #ndarray(12000,2,128)
                X.append(Xd2[(mod,snr)])    #ndarray(1000,2,128)
                for i in range(Xd1[(mod,snr)].shape[0]+Xd2[(mod,snr)].shape[0]):
                    lbl.append((mod,snr))
            train_idx+=list(np.random.choice(range(a*7000,(a+1)*7000), size=5000, replace=False))
            val_idx+=list(np.random.choice(list(set(range(a*7000,(a+1)*7000))-set(train_idx)), size=1000, replace=False))
            a+=1

    X = np.vstack(X)                    #(1540000,2,128)  mods * snr * 1000,total 154000 samples
